...
def _pipe_line_with_colons(colwidths, colaligns):
    ...
    segments = [_pipe_segment_with_colons(a, w) for a, w in
                zip(colaligns, colwidths)]
...
def _isint(string):
    ...
    return type(string) is int or \
           (isinstance(string, _binary_type) or isinstance(string,
                                                           _text_type)) and \
...
def _type(string, has_invisible=True):
    ...
    if has_invisible and \
            (isinstance(string, _text_type) or isinstance(string,
                                                          _binary_type)):
...
def tabulate(tabular_data, headers=[], tablefmt="simple",
    ...
    plain_text = '\n'.join(['\t'.join(map(_text_type, headers))] + \
                           ['\t'.join(map(_text_type, row)) for row in
                            list_of_lists])
    ...
    minwidths = [width_fn(h) + 2 for h in headers] if headers else [0] * len(
        cols)
    ...
    if headers:
        ...
        minwidths = [max(minw, width_fn(c[0])) for minw, c in
                     zip(minwidths, cols)]
